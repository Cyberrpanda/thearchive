import React from "react";

const Lock = ({ className, stroke = "currentColor", width = 80, height = 80 }) => {
  return (
    <svg
      className={className}
      fill="grey"
      stroke={stroke}
      width={width}
      height={height}
      viewBox="0 0 72.6 125"
      xmlns="http://www.w3.org/2000/svg"
    >
       <path d="M59.6,43.2h-8.7V28.6c0-8-6.5-14.6-14.6-14.6c-8,0-14.6,6.5-14.6,14.6v14.6H13V28.6C13,15.7,23.4,5.3,36.3,5.3  c12.8,0,23.3,10.5,23.3,23.3V43.2z M67.4,72.6c0,0.4-0.1,0.8-0.1,1.2c0.1,0.6,0.1,1.1,0.1,1.7v8.6c0,6.1-5,11-11,11H16.1  c-6.1,0-11-5-11-11v-8.6c0-0.6,0.1-1.1,0.1-1.7c-0.1-0.4-0.1-0.8-0.1-1.2v-21c0-3,2.5-5.5,5.5-5.5h51.4c3,0,5.5,2.5,5.5,5.5V72.6z   M40.5,69.1c1.4-1.2,2.3-3,2.3-5c0-3.6-2.9-6.6-6.6-6.6c-3.6,0-6.6,2.9-6.6,6.6c0,2,0.9,3.8,2.3,5l-1.7,10.5c0,2.2,3.7,4,5.9,4  c2.2,0,5.9-1.8,5.9-4L40.5,69.1z"/>
    </svg>
  );
};

export default Lock;
